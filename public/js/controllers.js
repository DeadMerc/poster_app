    ...
    .controller('CategoryCtrl', function ($rootScope, $scope, $http, $routeParams, $location) {
        ...
        $scope.save = function () {
            ...
            if ($scope.id !== 'new') {
                //console.log($scope.user);
                //$rootScope.transform(
                $http.put('/api/v1/users/' + $scope.id, $scope.data, $rootScope.config)
                    .then(function (res) {
                        if (res.data.error == false) {
                ...
                $http.post('/api/v1/users',$rootScope.transform($scope.user), $rootScope.config)
                    .then(function (res) {
                        if (res.data.error == false) {
                            $rootScope.success('OK');
                            //$location.path('/users');
    ...
    .controller('UserCtrl', function ($rootScope, $scope, $http, $routeParams, $location, Upload, $timeout) {
        ...
        $scope.save = function () {
            ...
            if ($scope.id !== 'new') {
                ...
                //console.log($scope.data);
                console.log($scope.params.name+' Ctrl try save new row');
                $http.post('/api/v1/users',$rootScope.transform($scope.data), $rootScope.config)
                    .then(function (res) {
                        if (res.data.error == false) {
                            $rootScope.success('OK');
                            $location.path('/users');
